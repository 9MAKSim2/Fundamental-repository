...
def count_nodes(graph): # считает количество узлов
    amount_of_nodes = list(set([i for l in graph for i in l]))
    return amount_of_nodes, len(amount_of_nodes) 

def edge_exsist(current_node, next_node, graph): #проверка связи между двумя конкретными узлами
    for i in graph:
...
def initialize_matrix(amount_of_nodes): # создаёт пустую матрицу
    matrix = []
